/* Buy a lot. Own a room. Doodle the walls. Place furniture. */
(function () {
  const KEY = "ns-house-v1:";
  const LOTS = [
    { id: "a1", name: "Margin cottage", cost: 60, x: 620, y: 560 },
    { id: "a2", name: "Graph loft", cost: 80, x: 1320, y: 560 },
    { id: "a3", name: "Comic flat", cost: 95, x: 2020, y: 560 },
    { id: "b1", name: "Club suite", cost: 140, x: 620, y: 1180 },
    { id: "b2", name: "Gallery house", cost: 170, x: 1320, y: 1180 },
    { id: "b3", name: "Patron villa", cost: 220, x: 2020, y: 1180 }
  ];
  const FURN = [
  ...
  function injectPages() {
    ...
    if (load() && !state.pages.some(function (p) { return p.id === "home"; })) state.pages.push({ id: "home", name: "My room" });
  }
  function go(id) {
    ...
    if (id === "street") {
      state.places = LOTS.map(function (l) {
        return { id: l.id, name: l.name, kind: "sign", x: l.x, y: l.y, r: 160, hint: l.cost + " ink" };
      });
    } else {
      state.places = [
  ...
  function paint() {
    ...
    if (page === "street") {
      ...
      box.innerHTML = "<b>Street</b><div class='sub'>One room per doodle. Pay ink. Doodle the walls.</div>" +
        LOTS.map(function (l) {
          const mine = h && h.lot === l.id;
          return "<button data-lot='" + l.id + "' " + (mine ? "disabled" : "") + ">" + l.name + " · " + (mine ? "yours" : l.cost + " ink") + "</button>";
        }).join("") + (h ? "<button data-home='1'>Go home</button>" : "");
      return;
    }
    if (page === "home" && h) {
      box.classList.add("show");
      box.innerHTML = "<b>" + h.title + "</b><div class='sub'>" + House.mode + " · " + (h.strokes || []).length + " doodles</div>" +
  ...
  window.drawHouses = function (g) {
    ...
    if (state.me.page === "street") {
      ...
      LOTS.forEach(function (lot) {
        ...
        g.fillText(h && h.lot === lot.id ? "YOURS" : lot.cost + " ink", lot.x, lot.y + 80);
      });
    }
    if (state.me.page === "home") {
  ...
  window.houseChat = function (raw) {
    ...
    if (t === "/home") { if (load()) go("home"); else if (typeof logLine === "function") logLine("Buy a lot on Street first."); return true; }
  ...
  function bind() {
    ...
    if (btn) btn.onclick = function () { if (load()) go("home"); else go("street"); };
    const box = document.getElementById("house-panel");
    ...
    if (canvas) {
      canvas.addEventListener("mousedown", function (e) {
        if (!state || state.me.page !== "home") return;
